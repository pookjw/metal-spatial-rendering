...
public:
    ...
    void renderFrame() {
        ...
        ar_device_anchor_t deviceAnchor = createDeviceAnchorForTiming(actualTiming);
...
private:
    ...
    ar_device_anchor_t createDeviceAnchorForTiming(cp_frame_timing_t timing) {
        ...
        ar_device_anchor_query_status_t status = ar_world_tracking_provider_query_device_anchor_at_timestamp(_worldTrackingProvider, queryTime, deviceAnchor);
        if (status != ar_device_anchor_query_status_success) {
            NSLog(@"Failed to get estimated device anchor from world tracking provider for presentation timestamp %0.3f", queryTime);
        }
        return deviceAnchor;
    }
